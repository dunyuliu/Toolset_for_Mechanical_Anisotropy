...
function [theta] = funcPlotCase35StressStrainAxes(path, theta, nn, LineStyle, figId, len1, StressFuncSpace)
    ...
    %%
    dx                 = 1/nn; % spatial discretization
    ...
    % creating interpolating points for a profile along y that passes through
    % the anisotropic layer. 

    % the final 1D profile array
    x_arr              = linspace(0,len2,nn);
    n_x_arr            = length(x_arr);
    [xq,yq,zq]         = meshgrid(len1/2,x_arr, len3/2);
    ...
    for ix = 1:m
        ...
        xticks([10,20,30,40,50,60,70,80,90]); % theta
        ...
        if cy<0.9 && cy>0.5
            xshift = 2/90; % shift 2 degs up.
            text(cx+xshift,cy,sprintf('%.2f',mis), 'FontWeight','bold'); hold on;
        end
        title('Principal Stress (black) vs Strain Rate (red)');
    end
    deg = [10,20,30,40,50,60,70,80,90]; % theta
    sstr = string(deg);
    xticks(deg/90);
    xticklabels(sstr);
    xlabel('\theta'); ylabel('Y'); zlabel('Z');
